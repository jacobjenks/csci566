...
def createNewHIT(imageURL)
  ...
  questionFile = "questions/food_0.1.question"
...
result = db.prepare("SELECT * FROM image WHERE id NOT IN(SELECT i.id FROM image i JOIN hit h ON i.id=h.image_id WHERE h.task_tier = 1) AND id=3").execute

i = 0
result.each do |row|
	i = i + 1
	hit = createNewHIT(row[1])
	db.prepare("INSERT INTO hit VALUES ('#{row[0]}', '1', '#{hit[:HITId]}')").execute
end
